...
public Controller::RestInstance<CrmCompanyController, CrmCompany> { 
  public:
    ...
    static constexpr std::string_view rest_actions[]= { "index", "read", 
      "delete", "multiple_update", "multiple_delete" };
  ...
  private:
    ...
    // across the models
    template <typename T>
    void modelUpdateTimestamp(T &model, Controller::PostBody &post, std::tm &tm_time) {
    ...
    void modelUpdate(CrmCompany &model, Controller::PostBody &post, std::tm tm_time) {
      modelUpdateTimestamp(model, post, tm_time);
